...
import { extractTableOfContents, guideHasTranslation, getGuideBySlug } from '@/lib/guides';
...
// The localized blog surface is intentionally runtime-rendered. Pre-rendering
// every post in every locale makes the content-heavy MDX route exceed the
// production worker timeout, while runtime rendering remains indexable through
// the localized sitemap and preserves the canonical/hreflang metadata below.
...
function getArticleNextStep(slug: string, dict: ReturnType<typeof getDictionary>) {
  if (slug.includes('fbar')) {
    return { href: '/tools/fbar-checker', label: dict.common.checkFbarStart, detail: 'Answer four threshold questions before deciding what to do next.' };
  }
  if (slug.includes('feie') || slug.includes('remote-worker')) {
    return { href: '/tools/feie-calculator', label: dict.common.estimateFeieImpact, detail: 'Run an educational estimate using your income and housing costs.' };
  }
  if (slug.includes('streamlined') || slug.includes('delinquent') || slug.includes('amending')) {
    return { href: '/tools/catch-up-program', label: dict.common.reviewCatchUp, detail: 'Use the educational decision tool, then verify the path for your facts.' };
  }
  return { href: '/intake', label: dict.common.getFilingRecommendation, detail: 'Complete the three-minute intake for a written scope or referral recommendation.' };
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const hasLocalizedContent = locale === defaultLocale || postHasTranslation(slug, locale);
  const canonicalLocale = hasLocalizedContent ? locale : defaultLocale;
  const canonical = localizePath(`/blog/${slug}`, canonicalLocale);

  const hreflangUrls: Record<string, string> = {};
  for (const loc of locales) {
    if (loc !== defaultLocale && !postHasTranslation(slug, loc)) continue;
    const url = localizePath(`/blog/${slug}`, loc);
    hreflangUrls[loc === 'en' ? 'en-us' : loc] = `https://fileabroad.com${url}`;
  }
  hreflangUrls['x-default'] = `https://fileabroad.com${localizePath(`/blog/${slug}`, defaultLocale)}`;

  return {
    title: post.title,
    description: post.description,
    authors: [{ name: post.author }],
    robots: post.reviewRequired || !hasLocalizedContent
    ...
    openGraph: {
      ...
      authors: [post.author],
      tags: post.tags,
      url: `https://fileabroad.com${canonical}`,
    },
    twitter: {
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const relatedPosts = getRelatedPosts(slug, post.category, post.tags, 3, locale);
  const relatedGuides = getRelatedGuidesForPost(post.tags).map(
    (guide) => getGuideBySlug(guide.slug, locale) || guide
  );
  ...
  const nextStep = getArticleNextStep(slug, dict);
  const l = (path: string) => localizePath(path, locale);

  const articleSchema = !post.reviewRequired && hasLocalizedContent
    ? buildArticleSchema({
        title: post.title,
        description: post.description,
        author: post.author,
  ...
  const faqSchema =
    !post.reviewRequired && hasLocalizedContent && post.faqs?.length
  ...
  return (
    <PageShell locale={locale}>
      ...
      <article className="mx-auto max-w-6xl px-6">
        ...
        <Breadcrumbs
          items={[
            ...
            { label: post.title, href: l(`/blog/${slug}`) },
          ]}
        />
        ...
        <header className="mb-10 max-w-4xl border-b border-border pb-10">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground">
            <span className="flex items-center gap-2">
              ...
              <Link href={l('/about')} className="font-semibold text-secondary underline-offset-4 hover:underline">
                {post.author}
              </Link>
            </span>
            <span className="flex items-center gap-2">
              <Calendar className="w-5 h-5" />
              {post.dateModified ? `${dict.blog.updated} ${formatDate(post.dateModified, locale)}` : `${dict.blog.published} ${formatDate(post.date, locale)}`}
            </span>
            <span className="flex items-center gap-2">
        ...
        {post.reviewRequired && (
          <aside className="mb-8 rounded-lg border border-red-200 bg-red-50 p-4 text-sm leading-relaxed text-red-950">
            <strong>{dict.faq.editorialReview}</strong> {dict.faq.editorialReviewDesc}
          </aside>
        )}
        ...
        <div className="prose prose-lg prose-fileabroad max-w-none">
          <MarkdownContent source={introContent} locale={locale} />
          <aside className="not-prose my-9 grid gap-4 border-l-4 border-secondary bg-primary p-5 text-primary-foreground sm:grid-cols-[1fr_auto] sm:items-center md:p-6">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-secondary">{dict.common.usefulNextStep}</p>
            ...
            <Link href={l(nextStep.href)} data-analytics-event="cta_recommendation_click" data-cta-location="article_context" className="inline-flex items-center justify-center rounded-md bg-white px-5 py-3 font-bold text-primary hover:bg-surface-elevated">
              {dict.common.continue}
            </Link>
          </aside>
          {remainingContent && (
            <MarkdownContent source={remainingContent} locale={locale} />
          )}
        </div>

        {!post.reviewRequired && post.faqs?.length ? (
          <BlogFaqs faqs={post.faqs} title={dict.home.faqHeading} />
        ) : null}
        ...
        {post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-12 pt-8 border-t border-border">
            ...
            {post.tags.map((tag) => (
              <Link key={tag} href={l(`/topics/${getTagSlug(tag)}`)}>
          ...
          <section className="mt-12 border-t border-border pt-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {relatedGuides.map((guide) => {
                const href = locale !== defaultLocale && !guideHasTranslation(guide.slug, locale)
                  ? `/guides/${guide.slug}`
                  : l(`/guides/${guide.slug}`);

                return (
                  <Link key={guide.slug} href={href} className="group">
          ...
          <section className="mt-12 border-t border-border pt-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {relatedCountries.map((country) => (
                <Link key={`country-${country.slug}`} href={l(`/countries/${country.slug}`)} className="group">
                  <Card className="h-full border-border transition-colors hover:border-secondary/50">
                    <CardContent className="p-5">
              ...
              {relatedPersonas.map((persona) => (
                <Link key={`persona-${persona.slug}`} href={l(`/personas/${persona.slug}`)} className="group">
                  <Card className="h-full border-border transition-colors hover:border-secondary/50">
                    <CardContent className="p-5">
              ...
              {relatedFaqs.map((faq) => (
                <Link key={`faq-${faq.slug}`} href={l(`/faq/${faq.slug}`)} className="group">
                  <Card className="h-full border-border transition-colors hover:border-secondary/50">
                    <CardContent className="p-5">
        ...
        {['state-tax-obligations-americans-abroad', 'best-state-domicile-expats'].includes(slug) && (
          <StateTaxGuideLinks locale={locale} />
        )}

        {slug === 'feie-vs-foreign-tax-credit' && <CountryGuideLinks locale={locale} />}

        {/* Author CTA */}
        <Card className="mt-12 border-border">
          <CardContent className="p-8">
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              <Link href={l('/about')} className="flex-shrink-0">
                <Image
                  src="/headshot.jpg"
              ...
              <div className="flex-1">
                <h3 className="text-xl font-bold mb-2 font-sans text-foreground">
                  {dict.blog.aboutAuthor}
                </h3>
                <p className="text-base mb-4 text-muted-foreground">
                  <Link href={l('/about')} className="text-secondary hover:underline">
                    Chip Moreno
                  </Link>
                  {' '}{dict.blog.authorDescription}{' '}
                  <Link href={l('/consultation')} className="text-secondary hover:underline">
                    {dict.nav.consultations}
                  </Link>
                  {' '}{dict.common.or}{' '}
                  <Link href={l('/intake')} className="text-secondary hover:underline">
                    {dict.nav.intake}
                  </Link>
                  .
                </p>
                <WhatsAppButton text={dict.common.askQuestion} className="!py-3 !text-base" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <section className="max-w-6xl mx-auto px-6 mt-16">
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {relatedPosts.map((related) => (
              <Link key={related.slug} href={l(`/blog/${related.slug}`)} className="group">
      ...
      {/* Newsletter Signup */}
      <NewsletterSignup />
      {locale === 'en' && <EditorialSourceNote routePattern="/blog/*" />}
